// Загружаем переменные окружения ПЕРЕД импортом PrismaClient
import 'dotenv/config';

// @ts-ignore - Prisma Client генерируется динамически
...
// Prisma v7: используем драйвер-адаптер для подключения к Postgres
...
async function main() {
  ...
  // Чистим данные (для локального/dev окружения)
  ...
  // Создаем дефолтное расписание для врача (все дни недели, 08:00-17:00, слоты по 30 минут)
  if (doctor.doctor) {
  ...
  // Создаем пример тренажёра с v0 проектом
  ...
  // Создаем дополнительные тренажёры
  ...
  // Определяем текущую дату для всех сессий
  const now = new Date();

  // Назначаем тренажёр первому пациенту
  if (doctor.doctor && patient1.patient) {
    ...
    // Создаем тестовые сессии с результатами
    
    // Сессия 1: Отличный результат (сегодня, утро)
    ...
    // Ответы для сессии 1 (все верные)
    await Promise.all(
      Array.from({ length: 10 }, (_, i) =>
        prisma.testAnswer.create({
          data: {
            ...
            isCorrect: true,
            createdAt: new Date(session1Started.getTime() + (i + 1) * 72000), // ~72 секунды на вопрос
          },
        })
      )
    );
    ...
    // Сессия 2: Хороший результат (вчера, вечер)
    ...
    // Ответы для сессии 2 (8 верных, 2 неверных)
    const session2Answers = [
      true, true, true, false, true, true, true, false, true, true
    ];
    await Promise.all(
      session2Answers.map((isCorrect, i) =>
        prisma.testAnswer.create({
          data: {
            ...
            answer: { selected: `option-${i + 1}`, value: `Ответ ${i + 1}` },
            isCorrect,
            createdAt: new Date(session2Started.getTime() + (i + 1) * 90000), // ~90 секунд на вопрос
          },
        })
      )
    );
    ...
    // Сессия 3: Средний результат (3 дня назад)
    ...
    // Ответы для сессии 3 (5 верных, 5 неверных)
    const session3Answers = [
      true, false, true, false, true, false, true, false, true, false
    ];
    await Promise.all(
      session3Answers.map((isCorrect, i) =>
        prisma.testAnswer.create({
          data: {
            ...
            answer: { selected: `option-${i + 1}`, value: `Ответ ${i + 1}` },
            isCorrect,
            createdAt: new Date(session3Started.getTime() + (i + 1) * 120000), // ~120 секунд на вопрос
          },
        })
      )
    );
    ...
    // Сессия 4: Плохой результат (5 дней назад)
    ...
    // Ответы для сессии 4 (3 верных, 7 неверных)
    const session4Answers = [
      false, false, true, false, false, true, false, false, true, false
    ];
    await Promise.all(
      session4Answers.map((isCorrect, i) =>
        prisma.testAnswer.create({
          data: {
            ...
            answer: { selected: `option-${i + 1}`, value: `Ответ ${i + 1}` },
            isCorrect,
            createdAt: new Date(session4Started.getTime() + (i + 1) * 150000), // ~150 секунд на вопрос
          },
        })
      )
    );
    ...
    // Сессия 5: Хороший результат (неделя назад)
    ...
    // Ответы для сессии 5 (9 верных, 1 неверный)
    const session5Answers = [
      true, true, true, true, true, true, true, true, false, true
    ];
    await Promise.all(
      session5Answers.map((isCorrect, i) =>
        prisma.testAnswer.create({
          data: {
            ...
            answer: { selected: `option-${i + 1}`, value: `Ответ ${i + 1}` },
            isCorrect,
            createdAt: new Date(session5Started.getTime() + (i + 1) * 108000), // ~108 секунд на вопрос
          },
        })
      )
    );
    ...
    // Сессия 6: Средний результат (10 дней назад)
    ...
    // Ответы для сессии 6 (6 верных, 4 неверных)
    const session6Answers = [
      true, true, false, true, false, true, false, true, true, false
    ];
    await Promise.all(
      session6Answers.map((isCorrect, i) =>
        prisma.testAnswer.create({
          data: {
            ...
            answer: { selected: `option-${i + 1}`, value: `Ответ ${i + 1}` },
            isCorrect,
            createdAt: new Date(session6Started.getTime() + (i + 1) * 132000), // ~132 секунды на вопрос
          },
        })
      )
    );
    ...
    // Создаем пример записи в дневнике
  ...
  // Назначаем тренажёры второму пациенту и создаем сессии
  if (doctor.doctor && patient2.patient) {
    ...
    // Сессия для второго пациента (2 дня назад)
    ...
    // Ответы для сессии второго пациента (7 верных, 3 неверных)
    const session2Patient2Answers = [
      true, true, false, true, true, false, true, true, false, true
    ];
    await Promise.all(
      session2Patient2Answers.map((isCorrect, i) =>
        prisma.testAnswer.create({
          data: {
            ...
            answer: { selected: `option-${i + 1}`, value: `Ответ ${i + 1}` },
            isCorrect,
            createdAt: new Date(session2Patient2Started.getTime() + (i + 1) * 96000), // ~96 секунд на вопрос
          },
        })
      )
    );
  ...
  // Инициализируем дефолтное расписание для всех врачей, у которых его нет
